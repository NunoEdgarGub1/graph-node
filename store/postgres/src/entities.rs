...
/// deployment/subgraph are tracked in the `sgd_schemas` table.
///
...
// for dynamic tables.
use public::sgd_schemas;

#[derive(Queryable, QueryableByName, Debug)]
#[table_name = "sgd_schemas"]
struct Schema {
    id: i32,
    subgraph: String,
    name: String,
...
fn find_schema(
    ...
    Ok(sgd_schemas::table
        .filter(sgd_schemas::subgraph.eq(subgraph.to_string()))
        .first::<Schema>(conn)
        .optional()?)
}
...
impl Table {
    ...
    /// If `subgraph` does not have an entry in `sgd_schemas`, we assume
...
pub(crate) fn create_schema(
    ...
    let schemas: Vec<String> = diesel::insert_into(sgd_schemas::table)
        .values(sgd_schemas::subgraph.eq(subgraph_id.to_string()))
        .returning(sgd_schemas::name)
        .get_results(conn)?;
    ...
    // TODO: should we check that event_source is not "none"?
    let query = format!(
...
pub(crate) fn drop_schema(
    ...
    if let Some(schema) = info {
        ...
        conn.batch_execute(&*query)?;
        Ok(diesel::delete(sgd_schemas::table)
            .filter(sgd_schemas::subgraph.eq(schema.subgraph))
            .execute(conn)?)
    } else {
        Ok(0)
    }
